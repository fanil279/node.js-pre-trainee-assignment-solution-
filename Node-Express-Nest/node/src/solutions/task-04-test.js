...
class TodoServerTester {
    ...
    async testCreateTodo() {
        ...
        if (!todo.id || !todo.title || !todo.createdAt || !todo.updatedAt) {
            throw new Error('Created todo should have id, title, createdAt, updatedAt');
        }
    ...
    async testGetTodoById() {
        ...
        if (!todoId) {
            throw new Error("Cannot test GET by ID - POST didn't return valid todo with ID");
        }
        ...
        if (response.statusCode !== 200) {
            throw new Error(`GET /todos/:id should return 200, got ${response.statusCode}`);
        }
    ...
    async testUpdateTodo() {
        ...
        const todoId = createResponse.body.data?.id;
        if (!todoId) {
            throw new Error("Cannot test PUT - POST didn't return valid todo with ID");
        }
        ...
        const response = await this.makeRequest('PUT', `/todos/${todoId}`, updateData);
        ...
        if (response.statusCode !== 200) {
            throw new Error(`PUT /todos/:id should return 200, got ${response.statusCode}`);
        }
        ...
        if (updatedTodo.title !== 'Updated Todo Title' || updatedTodo.completed !== true) {
    ...
    async testDeleteTodo() {
        ...
        const todoId = createResponse.body.data?.id;
        if (!todoId) {
            throw new Error("Cannot test DELETE - POST didn't return valid todo with ID");
        }
        ...
        if (response.statusCode !== 200) {
            throw new Error(`DELETE /todos/:id should return 200, got ${response.statusCode}`);
        }
    ...
    async testErrorHandling() {
        ...
        try {
            const response400 = await this.makeRequest('POST', '/todos', 'invalid json');
    ...
    async testQueryFiltering() {
        ...
        const completedResponse = await this.makeRequest('GET', '/todos?completed=true');
    ...
    async testCORSHeaders() {
        ...
        if (!corsOrigin) {
            throw new Error('Response should include CORS Access-Control-Allow-Origin header');
        }

        if (!corsMethods) {
            throw new Error('Response should include CORS Access-Control-Allow-Methods header');
        }
    }
    ...
    async runAllTests() {
        ...
        try {
            await this.runTest('Server: Basic functionality', () => this.testServerBasics());
            await this.runTest('GET /todos: Get all todos', () => this.testGetAllTodos());
            await this.runTest('POST /todos: Create new todo', () => this.testCreateTodo());
            await this.runTest('GET /todos/:id: Get todo by ID', () => this.testGetTodoById());
            await this.runTest('PUT /todos/:id: Update todo', () => this.testUpdateTodo());
            await this.runTest('DELETE /todos/:id: Delete todo', () => this.testDeleteTodo());
            await this.runTest('Error: Error handling', () => this.testErrorHandling());
            await this.runTest('Query: Filtering support', () => this.testQueryFiltering());
    ...
    printResults() {
        ...
        console.log(
            '\n' + (failed === 0 ? '🎉 All tests passed!' : '🔧 Some tests need attention.'),
        );
