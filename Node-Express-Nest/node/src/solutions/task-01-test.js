...
class MessageSystemTester {
    ...
    testSendMessage() {
        ...
        const result = this.messenger.sendMessage('message', 'Test message', 'TestUser');

        if (!result || typeof result !== 'object') {
            throw new Error('sendMessage should return a message object');
        }

        if (!result.id || !result.type || !result.content || !result.timestamp || !result.sender) {
            throw new Error('Message object should have id, type, content, timestamp, and sender');
        }
    ...
    testSubscribeToType() {
        ...
        if (!alertReceived) {
            throw new Error('Alert-specific subscription should receive alert messages');
        }
    ...
    testMessagePersistence() {
        ...
        const hasRecentMessage = history.some((msg) => msg.content === 'Message 105');
        if (!hasRecentMessage) {
    ...
    async runAllTests() {
        ...
        await this.runTest('sendMessage - basic functionality', () => this.testSendMessage());
        ...
        await this.runTest('removeUser - user removal', () => this.testRemoveUser());
        await this.runTest('getMessageHistory - message retrieval', () =>
            this.testGetMessageHistory(),
        );
        await this.runTest('getStats - statistics generation', () => this.testGetStats());
